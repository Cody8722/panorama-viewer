...
def get_albums():
    ...
    try:
        ...
        for album in albums:
            ...
            album['panorama_count'] = len(album.get('panorama_ids', []))

        return jsonify(albums), 200
    except Exception as e:
        logging.error(f"取得合輯列表失敗: {e}")
        return jsonify({'error': str(e)}), 500
...
def create_album():
    ...
    try:
        ...
        album = {
            ...
            'panorama_ids': data.get('panorama_ids', []),
            'created_at': datetime.now()
        }

        result = albums_collection.insert_one(album)
        ...
        logging.error(f"創建合輯失敗: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/albums/<album_id>', methods=['GET'])
def get_album(album_id):
    """取得單個合輯及其全景圖詳細資訊"""
    if albums_collection is None or panoramas_collection is None:
        return jsonify({'error': '資料庫未初始化'}), 500
    ...
    try:
        ...
        if not album:
            return jsonify({'error': '找不到合輯'}), 404

        # 取得合輯中所有全景圖的詳細資訊
        panorama_ids = [ObjectId(pid) for pid in album.get('panorama_ids', [])]
        panoramas = list(panoramas_collection.find(
            {'_id': {'$in': panorama_ids}},
        ...
        # 按照合輯中的順序排列
        panorama_dict = {str(p['_id']): p for p in panoramas}
        ordered_panoramas = []
        for pid in album.get('panorama_ids', []):
            if pid in panorama_dict:
                p = panorama_dict[pid]
                p['_id'] = str(p['_id'])
                p['created_at'] = p['created_at'].isoformat()
                ordered_panoramas.append(p)

        # 轉換資料格式
        album['_id'] = str(album['_id'])
        album['created_at'] = album['created_at'].isoformat()
        album['panoramas'] = ordered_panoramas

        return jsonify(album), 200

    except Exception as e:
        logging.error(f"取得合輯失敗: {e}")
        return jsonify({'error': str(e)}), 500
...
def update_album(album_id):
    ...
    try:
        ...
        if 'description' in data:
            update_data['description'] = data['description']
        if 'panorama_ids' in data:
            update_data['panorama_ids'] = data['panorama_ids']
        ...
        return jsonify({'message': '合輯已更新'}), 200

    except Exception as e:
        logging.error(f"更新合輯失敗: {e}")
        return jsonify({'error': str(e)}), 500
...
def delete_album(album_id):
    ...
    try:
        ...
        return jsonify({'message': '合輯已刪除'}), 200

    except Exception as e:
        logging.error(f"刪除合輯失敗: {e}")
        return jsonify({'error': str(e)}), 500
